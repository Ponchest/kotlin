...
val gradleVersionTaskGroup = "Kotlin Gradle Plugin Verification by Gradle Version"

val kgpAllTestsTask = tasks.register<Test>("kgpAllTests") {
...
val kgpByGradleVersionTestTasks = gradleVersions.map { gradleVersion ->
    tasks.register<Test>("kgpAllTestsForGradle_${gradleVersion.replace(".", "_")}") {
        group = gradleVersionTaskGroup
        description = "Runs all tests for Kotlin Gradle plugins"
        maxParallelForks = maxParallelTestForks
...
val kgpAllTestsByGradleVersionTask = tasks.register("kgpAllTestsByGradleVersion") {
    group = gradleVersionTaskGroup
    dependsOn(kgpByGradleVersionTestTasks)
}
