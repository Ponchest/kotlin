...
internal class ImplementationPrinter(printer: SmartPrinter) : AbstractImplementationPrinter<Implementation, Element, Field>(printer) {
    ...
    override fun SmartPrinter.printAdditionalMethods(implementation: Implementation) {
        if (implementation.element.let { it == IrTree.propertyWithLateBinding || it == IrTree.functionWithLateBinding }) {
            val symbolField = implementation.allFields.single { it.name == "symbol" }

            println()
            println("private var _symbol: ${symbolField.typeRef.render()}? = null")
            println()
            print()
            println(
                """
            override fun acquireSymbol(symbol: ${symbolField.typeRef.render()}): ${implementation.element.render()} {
        ...
        } else if (implementation.element.traverseParentsUntil { it == IrTree.symbolOwner }) {
            val symbolField = implementation.allFields.find { it.name == "symbol" }
            if (symbolField != null) {
                println()
                print("init")
                printBlock {
